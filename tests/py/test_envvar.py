...
class EnvVarTest(unittest.TestCase):
    def test_require_nonempty_rejects_missing_value(self):
        with patch.dict("os.environ", {}, clear=True):
            with self.assertRaisesRegex(RuntimeError, "GITHUB_RUN_ID is required"):
                env("GITHUB_RUN_ID").require_nonempty()

    def test_require_nonempty_rejects_empty_value(self):
        with patch.dict("os.environ", {"GITHUB_RUN_ID": ""}, clear=True):
            with self.assertRaisesRegex(RuntimeError, "GITHUB_RUN_ID must not be empty"):
                env("GITHUB_RUN_ID").require_nonempty()
